...
$arrayFile = explode('Estimated<br />Arrival Time</a></th></tr>',$rawFile);
$arrayFile = explode('<span style="font-size: x-small">&nbsp;',$arrayFile[1]);
$arrayFile = explode('</table>',$arrayFile[0]);
$arrayFile = explode('</thead>',$arrayFile[0]);
$arrayFile = explode('</tr>',$arrayFile[1]);
...
print_r($fieldArray);
echo "</pre>";
foreach ($fieldArray as $testOne) {
	echo strip_tags($testOne)."<br>";
}
exit();*/

	$flightNumber = strip_tags($fieldArray[0]);
	echo $flightNumber;
	exit();
