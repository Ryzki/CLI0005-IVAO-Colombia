...
############ Configuration ##############
$thumb_square_size 		= 1200; //Thumbnails will be cropped to 200x200 pixels
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if($image_res){
		...
		if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
		{
			...
			if ($db->connect_errno > 0) {
				die('Unable to connect to database [' . $db->connect_error . ']');
			}						$sql1 = "insert into eventos (nombre,hora_inicio,hora_fin,fecha,informacion,imagen,staff)                    values ('$nombresa','$horauno','$horados','$fecha','$info','$new_file_name','$id');";				if (!$result = $db->query($sql1)) {					die('There was an error running the query [' . $db->error . ']');				}			
			$sql = "update gvausers set pilot_image='".$new_file_name."' where gvauser_id=$id";
		...
		$db->close();
		echo '<META HTTP-EQUIV="Refresh" CONTENT="0; URL=./?page=eventos">';
	}
}
